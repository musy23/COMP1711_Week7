...
int main()
{
    ...
    while (fgets(line, buffer_size, input)) // while file is not empty get next line
    {
    ...
    while (1)
    {
        ...
        case 'b':
            ...
            for (int i = 0; i < numberOfElements; i++)
            {
                // use i instead of counter or numberOfElements 
                mean += daily_readings[i].bloodIron;
                // removed numberOfElements++; as it isn't needed. i increments anyway, i++ above
            }
